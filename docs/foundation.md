...
**Primary job: produce qualified interior-architecture enquiries.** Referral validation
is the mechanism that serves the existing branded pipeline (someone hears the name,
...
Priority order (confirmed as stated in the method):

1. **Private architecture clients.** Personas unchanged from the brand strategy:
   Isabelle (40, full renovation, ~150k), Bernard (60, modernisation, ~100k), the
...
- **No prices.** The internal price architecture (July 2026, supersedes the March
  docs): entry point around 200-350€ and not guaranteed (depends on the pieces
  artists bring), core at 500-800€, real presence above 1,000€ and into multiple
  thousands, ceiling well under 10k. Because the entry point is not guaranteed, no
  printed floor pre-opening. Accessibility is carried by the scene, the thesis line,
  and gift-register framing. Real ranges get added post-opening as a dated amendment.
- **No event promises** (vernissages, live painting: ambitions for after the first
  three months, closed decision), **no artist lineup or names** (consent not secured),
  **no ecommerce or catalogue, ever** (closed decision).
...
The five tone axes are closed (brand strategy) and carry unchanged: warm but not soft,
colourful but not loud, accessible but not cheap, expert but not academic, rock but
not punk. Philosophy line: "Chaque contrainte est un terrain de jeu." The never-list
...
- **2026-07-02, v1.1 (critic-pass reconciliation).** Applied the cross-model review:
  clarified that the pre-opening price ban covers art prices only (architecture
  pricing stays public); downgraded the project/B2B proof from "verified" to "exists,
  pending Crystelle's selection and permissions"; added the H1 implementation
  guardrail; flagged the exact shop French ("L'art shop", the thesis line) for
  Crystelle's validation. Also recorded two updates from Martin: the premium-inversion
