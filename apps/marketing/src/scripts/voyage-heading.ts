...
export function animateVoyageHeadings(gsap: typeof Gsap) {
  const phrases = document.querySelectorAll<HTMLElement>(
    '[data-voyage-heading]',
  );
  gsap.registerPlugin(CustomEase, SplitText);
  const settle = CustomEase.create('voyage-letter', '0.2,0.75,0.35,1');
  const media = gsap.matchMedia();
  phrases.forEach((phrase) => {
    media.add('(prefers-reduced-motion: no-preference)', () => {
      // Deep links and restored positions should arrive at readable text immediately.
      if (
        phrase.dataset.voyageRevealed ||
        phrase.getBoundingClientRect().top < innerHeight * 0.82
      ) {
        phrase.dataset.voyageRevealed = 'true';
        return;
      }
      const lines = [
        ...phrase.querySelectorAll<HTMLElement>('[data-voyage-line]'),
      ];
      const entrances = lines.map((line, lineIndex) => {
        ...
        const tween = gsap.to(split.chars, {
          ...
          duration: 1.3,
          ease: settle,
          // Scatter arrivals over 650ms, with a repeatable order for each line.
          // Neighbouring letters must remain at visibly different stages.
          stagger: (index) => ((index * 7 + lineIndex * 3) % 11) * 0.065,
          scrollTrigger: {
            trigger: line,
            start: 'top 82%',
            once: true,
